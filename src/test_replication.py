...
Each test requires that the difference is within 50 bps for at least 90% of valid observations.
"""
...
def my_irr_df():
    ...
    path = DATA_MANUAL / "implied_repo_first_deferred.parquet"
    if not path.exists():
...
def my_spread_df():
    ...
    path = DATA_MANUAL / "arbitrage_spreads.parquet"
    if not path.exists():
        try:
            df = calc_arbitrage_spread()
        except Exception as e:
...
def test_irr_vs_tfut_rf_all_tenors(paper_df, my_irr_df):
    """Spread between my IRR and tfut_*_rf is within 50 bps for >= 90% of data, for all 5 tenors."""
    ...
    for tenor in TENORS:
        ...
        frac = _frac_within_bps(my_a[my_col], paper_a[paper_col], BPS_TOLERANCE)
        assert frac >= MIN_FRACTION_WITHIN, (
            f"IRR vs tfut_*_rf for {tenor}: {frac:.2%} of observations within {BPS_TOLERANCE} bps "
            f"(required >= {MIN_FRACTION_WITHIN:.0%})"
        )


def test_ois_vs_tfut_ois_all_tenors(paper_df, my_ois_df):
    """Difference between my OIS and tfut_ois_* is within 50 bps for >= 90% of data, for all 5 tenors."""
    ...
    for tenor in TENORS:
        ...
        frac = _frac_within_bps(my_a[my_col], paper_a[paper_col], BPS_TOLERANCE)
        assert frac >= MIN_FRACTION_WITHIN, (
            f"OIS vs tfut_ois_* for {tenor}: {frac:.2%} of observations within {BPS_TOLERANCE} bps "
            f"(required >= {MIN_FRACTION_WITHIN:.0%})"
        )


def test_spread_vs_paper_spread_all_tenors(paper_df, my_spread_df):
    """Spread I compute vs spread_* is within 50 bps for >= 90% of data, for all 5 tenors."""
    ...
    for tenor in TENORS:
        ...
        frac = _frac_within_bps(my_a[my_col], paper_a[paper_col], BPS_TOLERANCE)
        assert frac >= MIN_FRACTION_WITHIN, (
            f"My spread vs spread_* for {tenor}: {frac:.2%} of observations within {BPS_TOLERANCE} bps "
            f"(required >= {MIN_FRACTION_WITHIN:.0%})"
        )
